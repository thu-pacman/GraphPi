...
TEST(performance_test, performance_patents_qg5) {
    Graph *g;
    DataLoader D;
    ...
    int dataset_vertex_size = 3774768;
    int dataset_edge_size = 16518948;
    int size = 5;
    ...
    do{
        ...
        pattern.add_edge(std::min(rank[0], rank[4]), std::max(rank[0], rank[4])); 
        pattern.add_edge(std::min(rank[1], rank[4]), std::max(rank[1], rank[4])); 
        pattern.add_edge(std::min(rank[1], rank[2]), std::max(rank[1], rank[2])); 
        pattern.add_edge(std::min(rank[2], rank[3]), std::max(rank[2], rank[3])); 
        pattern.add_edge(std::min(rank[3], rank[4]), std::max(rank[3], rank[4])); 
